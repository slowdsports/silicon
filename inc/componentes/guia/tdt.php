...
$canales = [];
$canalesAgregados = []; // Array para rastrear los canales ya agregados
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!isset($canalesAgregados[$epgCanal])) {
        $canalesAgregados[$epgCanal] = true; // Marcar que este canal ya fue agregado
            ...
            if ($canalProgramacion['name'] === $epgCanal) {
                ...
                if (!empty($programasEnVivo)) {
                    $canales[$epgCanal] = [
                        ...
                        'pais' => $paisNombre,
                        'flag' => $paisFlag,
                        'programa' => $programasEnVivo[0] // Solo toma el primer programa en vivo
                    ];
                } else {
...
if (!empty($canales)) {
    ...
    echo "Se ha guardado la información de los canales con su programación en vivo en el archivo spain.json.";
} else {
